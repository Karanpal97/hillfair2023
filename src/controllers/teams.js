...
const getMembers = async (request, response) => {
    try {
        const members = await team_model.find();
...
const postTeamMember = async (req, res) => {
    ...
    try {
        const member = await team_model.create(event);
        res.status(201).json({ message: "added the team member", member });
    } catch (error) {
...
const getTeamMembers = async (req, res) => {
    ...
    try {
        if (year == 0) {
            ...
            const teamMembers = await member_model.find({
                teamId: teamId,
                year: year,
            });
            res.status(200).json(teamMembers);
...
const updateMember = async (req, res) => {
    ...
    try {
        const newMember = await member_model.findByIdAndUpdate(
            memberId,
            memberdata,
            {
                new: true,
